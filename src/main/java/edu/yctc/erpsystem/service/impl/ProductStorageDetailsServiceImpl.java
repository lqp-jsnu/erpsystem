...
import edu.yctc.erpsystem.excel.ProductStorageDetailsModel;
...
@Service("productStorageDetailsService")
public class ProductStorageDetailsServiceImpl implements ProductStorageDetailsInterService {
    ...
    @Override
    public ResultDO<PageUtils<ProductStorageDetailsVO>> getProductStorageDetails(Map<String, Object> params) {
        return CallbackUtils.getCallback("getProductStorageDetails", params.toString(), () -> {
            List<ProductStorageDetailsVO> result = new ArrayList<>();

            // 得到所有成品入库明细
            List<ProductStorageDO> prodStorageDetailsList = productStorageDAO.getProductStorageDetail(params);

            if (prodStorageDetailsList == null) {
                logger.error("getProductStorageDetails exception, db error, params={}", params);
                return new ResultDO<>(false, ResultCode.DB_ERROR, ResultCode.MSG_DB_ERROR, null);
            }
            // 如果没有记录直接返回
            if (0 == prodStorageDetailsList.size()) {
                return new ResultDO<>(true, ResultCode.SUCCESS, ResultCode.MSG_SUCCESS, new PageUtils<>(0, result));
            }

            // 得到所有制造流程单副表
            List<ManufactureProcessSlaveDO> manufactureProcessSlaveList = manufactureProcessSlaveDAO.getManufactureProcessSlaveByIds(prodStorageDetailsList.stream().map(ProductStorageDO::getManufactureProcessSlaveId).collect(Collectors.toList()));
            ...
            // 得到仓库信息
            List<WarehouseDO>  warehouseList= warehouseDAO.getWarehouseByIds(prodStorageDetailsList.stream().map(ProductStorageDO::getWareHouseId).collect(Collectors.toList()));
            Map<String, WarehouseDO> warehouseMap = warehouseList.stream().collect(Collectors.toMap(WarehouseDO::getId, warehouse->warehouse));

            // 得到所有录入者
            List<UserDO> enterList = userDAO.getUserByIds(prodStorageDetailsList.stream().map(ProductStorageDO::getUser).collect(Collectors.toList()));
            Map<String, UserDO> enterMap = enterList.stream().collect(Collectors.toMap(UserDO::getId, enter -> enter));

            for (ProductStorageDO tempProductStorage : prodStorageDetailsList) {
                ProductStorageDetailsVO temp = new ProductStorageDetailsVO();
            ...
            return new ResultDO<>(true, ResultCode.SUCCESS, ResultCode.MSG_SUCCESS, new PageUtils<>(productStorageDAO.countDetail(params), result));
        });
    }

    @Override
    public ResultDO<PageUtils<ProductStorageDetailsVO>> getProductStorageDetailsByConditions(Map<String, Object> params){
        return CallbackUtils.getCallback("getProductStorageDetailsByConditions", params.toString(), () ->{
            // 获得所有数据
            Integer offset = (Integer) params.get("offset");
            Integer limit = (Integer) params.get("limit");
            params.remove("offset");
            params.remove("limit");
            ResultDO<PageUtils<ProductStorageDetailsVO>> resultVO = getProductStorageDetails(params);

            if (resultVO == null) {
                logger.error("getProductStorageDetailsByConditions exception, db error, params={}", params);
                return new ResultDO<>(false, ResultCode.DB_ERROR, ResultCode.MSG_DB_ERROR, null);
            }
            ...
            List<ProductStorageDetailsVO> result = resultVO.getModule().getRows();
    ...
    @Override
    public ResultDO<Void> exportExcel(ProductStorageDetailsVO[] productStorageDetailsVO) {
        if (null == productStorageDetailsVO || 0 == productStorageDetailsVO.length) {
            return new ResultDO<>(false, ResultCode.PARAMETER_INVALID, ResultCode.MSG_PARAMETER_INVALID, null);
        }

        return CallbackUtils.exportCallback("ProdStorageDetails exportExcel", Arrays.toString(productStorageDetailsVO), () -> {
            List<ProductStorageDetailsModel> data = new ArrayList<>();

            int i = 1;
            for(ProductStorageDetailsVO temp : productStorageDetailsVO) {
                ProductStorageDetailsModel item=new ProductStorageDetailsModel();
            ...
            return MyExcel.write(ConstantHolder.FILE_SAVE_PATH + ConstantHolder.PRODUCT_STORAGE_DETAILS_FILE_NAME, ProductStorageDetailsModel.class, data, columnWidth, new MaterialDumpExcelStyleHandler());
        });
    }
